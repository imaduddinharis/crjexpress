...
<header class="header_area">
  ...
  <section class="section-padding--small bg-gray">
    <div class="container text-center">
      ...
      <p>Input Token Number</p>
      <?php if($page == 1):?>
      <form method="post" action="<?=base_url()?>feature-crj-express/listrik-pra-bayar" class="form-row mt-3">
        <!-- <center> -->
        <div class="col-2"></div>
        <div class="input-group mb-2 col-md-4 col-sm-4 col-12">
            <input type="number" class="form-control" name="phone" placeholder="Token Number">
        </div>
        <div class="input-group mb-2 col-md-3 col-sm-3 col-12">
            <select class="form-control" name="nominal" placeholder="08xxxxxxxx">
                <?php foreach($option as $data):?>
                  <option value="<?=$data->pulsa_code?>">nominal: <?=number_format($data->pulsa_nominal,0)?>, price: Rp.<?=number_format($data->pulsa_price,0)?></option>
                <?php endforeach;?>
            </select>
        </div>
        <div class="input-group mb-2 col-md-2 col-sm-2 col-12">
            <button class="btn btn-danger" type="submit" name="check">Next</button>
        </div>
        <!-- </center> -->
      </form>
      <?php elseif($page == 2):?>
        <?php if($custinfo->status == 1):?>
        <form method="post" action="<?=base_url()?>feature-crj-express/listrik-pra-bayar/topup" class="mt-3" onsubmit="return confirm('Do you really want to buy?');">
          <br>
          <p class="text-danger font-weight-bold mb-3 mt-3">
            Customer ID: <?=$custinfo->hp?><br>
            Meter Number: <?=$custinfo->meter_no?><br>
            Customer Name: <?=$custinfo->name?><br>
            Segment Power: <?=$custinfo->segment_power?>
          </p><br>
        ...
        <?php elseif($custinfo->status == 2):?>
          <br><p class="text-danger font-weight-bold mb-3 mt-3">INCORECT DESTINATION NUMBER</p><br>
          <a class="btn btn-danger" href="<?=base_url()?>feature-crj-express/listrik-pra-bayar">Back</a>
        <?php endif;?>
      <?php endif;?>
